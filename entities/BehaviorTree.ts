  ...
  return new Selector([
    ...
    new Sequence([
      ...
      new Selector([
        ...
        new Sequence([
          ...
          new Action((agent: AIPlayerEntity) => {
            ...
            for (const teammate of allTeammates) {
              ...
              // Prefer forward passes but allow backwards/sideways
              if (forwardProgress > 0) {
                score += forwardProgress * 2; // Forward progress is good
              } else {
                score += forwardProgress * 0.5; // Small penalty for backwards
              }

              // Prefer teammates closer to goal
              score += (100 - distanceToGoal);

              // Prefer moderate passing distances
              if (distanceToTeammate > 8 && distanceToTeammate < 20) {
                score += 20; // Bonus for good passing distance
              }
            ...
            if (bestTeammate) {
              ...
              const distToTeammate = agent.distanceBetween(agent.position, bestTeammate.position);

              // Velocity-aware passing
              ...
              // Reduced pass speed for better control
              ...
              if (success) {
                console.log(`${agent.player.username} (BT-Strategic) passing to teammate at (${passToPoint.x.toFixed(1)}, ${passToPoint.z.toFixed(1)}), margin=${safetyMargin.toFixed(1)}u`);
              }
              return success;
            }
        ...
        // Shoot (Within Scoring Range?) - CHECK SHOOTING SECOND (after passing)
        new Sequence([
          new Condition((agent: AIPlayerEntity) => {
            // Within Shooting Range check - REDUCED ranges for strategic play
            const opponentGoalLineX = agent.team === 'red' ? AI_GOAL_LINE_X_BLUE : AI_GOAL_LINE_X_RED;
            const goalPosition = { x: opponentGoalLineX, y: 1, z: AI_FIELD_CENTER_Z };

            // REDUCED shooting ranges to encourage passing
            let maxShootingRange = agent.aiRole === 'striker' ? 12 : 8; // Reduced from 20/15
            ...
            if (staminaPercentage < 30) {
              maxShootingRange *= 0.7;
            } else if (staminaPercentage < 50) {
              maxShootingRange *= 0.85;
            }
        ...
        // Dribble Forward - Default action when no pass/shoot available
        new Action((agent: AIPlayerEntity) => {
          // Dribble action
