...
export default class AIPlayerEntity extends SoccerPlayerEntity {
  ...
  private makeDecision() {
    ...
    // **GOALKEEPER RESPECT ZONE**: Check if opponent's goalkeeper has the ball
    // If so, back off and position to intercept passes instead of pressing
    ...
    if (playerWithBall && playerWithBall !== this &&
        ...
        playerWithBall.role === 'goalkeeper') {

      // Opposing goalkeeper has possession - BACK OFF and position strategically
      this.handleOpponentGoalkeeperPossession(playerWithBall);
      return; // Skip normal aggressive decision making
    }
  ...
  public shootBall(targetPoint: Vector3Like, powerMultiplier: number = 1.0): boolean {
    ...
    // DISTANCE-BASED FORCE SCALING (Realistic Soccer Physics - Option 3)
    // Close shots: Fast and powerful | Long shots: Slower with more arc
    let distanceScaledForce = SHOT_FORCE;
    if (distanceHorizontal < 10) {
      // Close range (6-10m): Quick strikes, tap-ins - 20% more power
      distanceScaledForce = SHOT_FORCE * 1.2;
      console.log(`🎯 Close-range shot (${distanceHorizontal.toFixed(1)}m) - Fast & low`);
    } else if (distanceHorizontal > 20) {
      // Long range (20m+): Slower, spectacular arcing shots - 30% less power
      distanceScaledForce = SHOT_FORCE * 0.7;
      console.log(`🚀 Long-range shot (${distanceHorizontal.toFixed(1)}m) - Slow & arc`);
    } else {
      // Medium range (10-20m): Standard shots
      console.log(`⚽ Medium-range shot (${distanceHorizontal.toFixed(1)}m) - Balanced`);
    }

    // DISTANCE-BASED ARC SCALING (More arc for longer shots)
    // Long shots should have beautiful parabolic arcs like real soccer
    const arcMultiplier = Math.min(distanceHorizontal / 15, 2.0); // Up to 2x arc for long shots
    const baseArc = distanceHorizontal * SHOT_ARC_FACTOR * arcMultiplier;
    const distanceBonus = Math.min(distanceHorizontal / 25, 1.2) * 1.2; // Extra arc bonus for very long shots
    ...
    // Apply powerMultiplier to the distance-scaled force (still respect multiplier for special shots)
    const effectiveMultiplier = powerMultiplier; // Use the passed multiplier (usually 1.0 or 1.2)

    // Calculate the effective shot force using distance-scaled force
    const effectiveShotForce = Math.min(distanceScaledForce * effectiveMultiplier, 10); // Safety cap

    // INCREASED vertical force cap for beautiful dipping shots (Option 3)
    const verticalComponent = direction.y * effectiveShotForce;
    const maxVerticalForce = 6.5; // INCREASED from 4.0 to 6.5 - allows spectacular top-corner goals
  ...
  public passBall(): boolean {
    ...
    if (bestTargetPlayer) {
      // VELOCITY-AWARE PASS LEADING: Fixed double-leading issue (consistent with SoccerAgent/BehaviorTree)
      ...
      if (passDist > 0) {
        ...
        // Check if teammate is moving (has velocity)
        let teammateVelocity = { x: 0, z: 0 };
        if (bestTargetPlayer instanceof SoccerPlayerEntity && bestTargetPlayer.linearVelocity) {
          teammateVelocity = {
            x: bestTargetPlayer.linearVelocity.x,
        ...
        // SAFETY MARGIN: Small buffer instead of large static lead
        let safetyMargin = 0.8; // Base safety margin
        if (passDist < 10) {
          safetyMargin = 0.5; // Short passes: minimal margin
        } else if (passDist > 20) {
          safetyMargin = 1.2; // Long passes: more margin (was 4.0+!)
        }
  ...
  /**
   * GOALKEEPER RESPECT ZONE: Handle behavior when opposing goalkeeper has possession
   * Instead of pressing the goalkeeper aggressively, attackers back off and position
   * to intercept passes to defenders/midfielders
   * @param opponentGoalkeeper The opposing goalkeeper with possession
   */
  private handleOpponentGoalkeeperPossession(opponentGoalkeeper: SoccerPlayerEntity): void {
    const GOALKEEPER_RESPECT_DISTANCE = 15; // Minimum distance to maintain from goalkeeper
    const INTERCEPT_POSITION_DISTANCE = 20; // How far to position for interception

    const gkPosition = opponentGoalkeeper.position;
    const myPosition = this.position;
    const distanceToGK = this.distanceBetween(myPosition, gkPosition);

    // Get opponent's likely passing targets (their defenders/midfielders)
    const opponentTeam = opponentGoalkeeper.team;
    const opponentAITeam = opponentTeam === 'red' ? sharedState.getRedAITeam() : sharedState.getBlueAITeam();
    ...
    if (this.aiRole === 'striker') {
      // STRIKERS: Position to intercept passes to opponent's defenders
      // Find opponent's fullbacks/center-backs as likely pass targets
      const opponentDefenders = opponentAITeam.filter(p =>
        p.isSpawned && (p.aiRole === 'left-back' || p.aiRole === 'right-back')
      );

      if (opponentDefenders.length > 0) {
        // Position between goalkeeper and their nearest defender
        const nearestDefender = opponentDefenders.reduce((nearest, defender) => {
          const distToGK = this.distanceBetween(gkPosition, defender.position);
          const nearestDistToGK = this.distanceBetween(gkPosition, nearest.position);
          return distToGK < nearestDistToGK ? defender : nearest;
        });

        // Position 60% of the way between GK and defender (closer to defender for interception)
        this.targetPosition = {
          x: gkPosition.x + (nearestDefender.position.x - gkPosition.x) * 0.6,
          y: myPosition.y,
    ...
    } else if (this.aiRole === 'central-midfielder-1' || this.aiRole === 'central-midfielder-2') {
      // MIDFIELDERS: Position to cut off passing lanes to opponent midfield
      const opponentMidfielders = opponentAITeam.filter(p =>
        p.isSpawned && (p.aiRole === 'central-midfielder-1' || p.aiRole === 'central-midfielder-2')
      );

      if (opponentMidfielders.length > 0) {
        // Find the opponent midfielder on the same side
        const sameSideMidfielder = opponentMidfielders.find(m =>
          this.aiRole === 'central-midfielder-1' ? m.aiRole === 'central-midfielder-1' : m.aiRole === 'central-midfielder-2'
        ) || opponentMidfielders[0];

        // Position to cut off passing lane
        this.targetPosition = {
          x: gkPosition.x + (sameSideMidfielder.position.x - gkPosition.x) * 0.5,
          y: myPosition.y,
          z: gkPosition.z + (sameSideMidfielder.position.z - gkPosition.z) * 0.5
        };
      } else {
        // Drop back to defensive position
        const formationPos = this.getRoleBasedPosition();
        const defensiveAdjust = this.team === 'red' ? -8 : 8; // Pull back toward own goal
        this.targetPosition = {
    ...
    } else if (this.aiRole === 'left-back' || this.aiRole === 'right-back') {
    ...
    // Ensure we maintain minimum respect distance from opponent goalkeeper
    if (distanceToGK < GOALKEEPER_RESPECT_DISTANCE) {
      this.maintainRespectDistance(gkPosition, GOALKEEPER_RESPECT_DISTANCE);
    }

    // Apply spacing to avoid crowding teammates
    this.targetPosition = this.adjustPositionForSpacing(this.targetPosition);
  }

  /**
   * Helper method to maintain a minimum respect distance from opponent goalkeeper
   * Pushes the AI player away if they're too close
   */
  private maintainRespectDistance(gkPosition: Vector3Like, minDistance: number): void {
    const myPosition = this.position;
    const distanceToGK = this.distanceBetween(myPosition, gkPosition);

    if (distanceToGK < minDistance) {
      // Calculate direction away from goalkeeper
      const awayDirection = {
        x: myPosition.x - gkPosition.x,
        z: myPosition.z - gkPosition.z
      };
      ...
      if (length > 0.1) {
        const normalized = {
          x: awayDirection.x / length,
          z: awayDirection.z / length
        };

        // Move to maintain minimum distance
        this.targetPosition = {
          x: gkPosition.x + normalized.x * minDistance,
          y: myPosition.y,
