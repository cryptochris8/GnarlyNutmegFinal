...
export class SoccerAgent {
   ...
   * GOALKEEPER ENHANCEMENT: Special distribution logic for goalkeepers
   */
  public passBall(): boolean {
    if (!this.hasBall()) return false;

    // GOALKEEPER DISTRIBUTION MODE: Special passing logic for safe distribution
    if (this.entity.aiRole === 'goalkeeper') {
      return this.goalkeeperDistribution();
    }
    ...
    if (bestTeammate) {
      // IMPROVED PASS LEADING: Velocity prediction with small safety margin (fixed double-leading issue)
      ...
      // VELOCITY-AWARE PASSING: Account for teammate movement
      ...
      if (distToTeammate > 0.1) {
        ...
        if (bestTeammate instanceof SoccerPlayerEntity && bestTeammate.linearVelocity) {
          teammateVelocity = {
            x: bestTeammate.linearVelocity.x,
            z: bestTeammate.linearVelocity.z
          };
        }

        // Estimate pass travel time based on distance (reduced pass speed for more control)
        const passSpeed = 2.8; // Reduced from 3.5 for better accuracy
        const passTravelTime = distToTeammate / passSpeed;

        // Predict where teammate will be when ball arrives
        const predictedX = bestTeammate.position.x + (teammateVelocity.x * passTravelTime);
        const predictedZ = bestTeammate.position.z + (teammateVelocity.z * passTravelTime);

        // SAFETY MARGIN: Small buffer instead of large static lead (prevents double-leading)
        let safetyMargin = 0.8; // Base safety margin
        if (distToTeammate < 10) {
          safetyMargin = 0.5; // Short passes: minimal margin
        } else if (distToTeammate > 20) {
          safetyMargin = 1.2; // Long passes: more margin (was 4.0!)
        }

        // Use predicted position + small safety margin (NOT large static lead)
        passToPoint = {
          x: predictedX + normDirX * safetyMargin,
          y: bestTeammate.position.y,
      ...
      // DISTANCE-BASED PASS POWER (realistic passing strength)
      let passPowerMultiplier = 1.0;
      if (distToTeammate < 10) {
        passPowerMultiplier = 0.7; // Short passes: softer touch
      } else if (distToTeammate > 20) {
        passPowerMultiplier = 1.0; // Long passes: full power
      } else {
        passPowerMultiplier = 0.85; // Medium passes: moderate power
      }
  ...
  /**
   * GOALKEEPER DISTRIBUTION: Special passing logic for goalkeepers after saves
   * Prioritizes safe passing targets (defenders/midfielders in open space)
   */
  public goalkeeperDistribution(): boolean {
    ...
    if (this.entity.world) {
      ...
      for (const playerEntity of allPlayerEntities) {
        if (playerEntity instanceof SoccerPlayerEntity &&
            ...
            playerEntity.isSpawned &&
            !playerEntity.isPlayerFrozen &&
            !(playerEntity instanceof AIPlayerEntity)) {
          allTeammates.push(playerEntity);
        }
      }
    }
    ...
    for (const teammate of allTeammates) {
      ...
      // Goalkeeper distribution: Look for teammates within reasonable passing range
      if (distanceToTeammate < 5) continue; // Too close
      if (distanceToTeammate > 35) continue; // Too far for safe distribution

      let score = 0;

      // PRIORITY 1: Prefer defenders and midfielders (safe outlets)
      if (teammate instanceof AIPlayerEntity) {
        if (teammate.aiRole === 'left-back' || teammate.aiRole === 'right-back') {
          score += 50; // High priority for fullbacks
        } else if (teammate.aiRole === 'central-midfielder-1' || teammate.aiRole === 'central-midfielder-2') {
          score += 40; // Good priority for midfielders
        } else if (teammate.aiRole === 'striker') {
          score += 20; // Lower priority for strikers (risky long balls)
        }
      }

      // PRIORITY 2: Check for space around teammate (CRITICAL)
      let opponentsNearby = 0;
      let nearestOpponentDistance = Infinity;

      for (const opponent of opponents) {
        if (!opponent.isSpawned) continue;
        const distanceToOpponent = this.entity.distanceBetween(teammate.position, opponent.position);

        if (distanceToOpponent < nearestOpponentDistance) {
          nearestOpponentDistance = distanceToOpponent;
        }

        if (distanceToOpponent < 8) {
          opponentsNearby++;
        }
      }

      // Heavy penalty for crowded teammates
      if (opponentsNearby > 2) {
        score -= 60; // Very crowded - avoid
      } else if (opponentsNearby > 1) {
        score -= 30; // Somewhat crowded
      } else if (opponentsNearby === 0) {
        score += 40; // Wide open - excellent target!
      }

      // Bonus for more distance from nearest opponent
      score += nearestOpponentDistance * 3; // More space = better

      // PRIORITY 3: Prefer moderate passing distances (easier to complete)
      if (distanceToTeammate > 10 && distanceToTeammate < 25) {
        score += 25; // Sweet spot for goalkeeper passes
      }
    ...
    // Execute the pass to the best teammate
    if (bestTeammate && bestScore > -20) { // Require minimum viable score
      const dirToTeammateX = bestTeammate.position.x - this.entity.position.x;
      const dirToTeammateZ = bestTeammate.position.z - this.entity.position.z;
      const distToTeammate = Math.sqrt(dirToTeammateX * dirToTeammateX + dirToTeammateZ * dirToTeammateZ);

      // VELOCITY-AWARE DISTRIBUTION: Fixed double-leading issue
      let passToPoint: Vector3Like;
      if (distToTeammate > 0.1) {
        const normDirX = dirToTeammateX / distToTeammate;
        const normDirZ = dirToTeammateZ / distToTeammate;

        // Check teammate velocity for prediction
        let teammateVelocity = { x: 0, z: 0 };
        if (bestTeammate instanceof SoccerPlayerEntity && bestTeammate.linearVelocity) {
          teammateVelocity = {
            x: bestTeammate.linearVelocity.x,
        ...
        // Predict where teammate will be (reduced pass speed for control)
        const passSpeed = 2.8; // Reduced from 3.5
        const passTravelTime = distToTeammate / passSpeed;
        const predictedX = bestTeammate.position.x + (teammateVelocity.x * passTravelTime);
        const predictedZ = bestTeammate.position.z + (teammateVelocity.z * passTravelTime);

        // SAFETY MARGIN: Goalkeeper uses slightly larger margin for safety
        let safetyMargin = 1.0; // Base margin
        if (distToTeammate < 10) {
          safetyMargin = 0.6; // Short distribution: precise (was 1.8!)
        } else if (distToTeammate > 20) {
          safetyMargin = 1.5; // Long distribution: more margin (was 4.5!)
        }
      ...
      if (distToTeammate > 20) {
        passPower = 1.0; // Full power for long distribution
      }

      const success = this.entity.forcePass(bestTeammate, passToPoint, passPower);
      return success;
    }
