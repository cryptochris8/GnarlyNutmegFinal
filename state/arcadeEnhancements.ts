...
export class ArcadeEnhancementManager {
  ...
  constructor(world: World) {
    ...
    this.freezeSafetyInterval = this.registerTimer(
      setInterval(() => {
  ...
  private safeCreateAudio(uri: string, options: any = {}): Audio | null {
    try {
  ...
  private safePlayAudio(audio: Audio | null, world: any): boolean {
    if (!audio) {
  ...
  // Cleanup method to stop all timers and prevent crashes
  destroy(): void {
    if (this.isDestroyed) {
      console.warn("ArcadeEnhancementManager already destroyed");
      return;
    }

    this.isDestroyed = true;
    console.log("🧹 ArcadeEnhancementManager: Starting cleanup process...");
    ...
    console.log("✅ ArcadeEnhancementManager: Cleanup completed");
  }

  // Timer management methods to prevent accumulation and crashes
  private registerTimer(timer: NodeJS.Timeout, type: string, playerId?: string): NodeJS.Timeout {
    if (this.isDestroyed) {
      clearTimeout(timer);
      return timer;
    }
  ...
  private updatePlayerEnhancements(): void {
    ...
    // Get all player IDs to iterate safely
  ...
  private executeFreezeBlast(playerId: string): void {
    ...
    if (!playerEntity.isSpawned || !playerEntity.world) {
      console.error(`🧊 FREEZE BLAST: Player entity not properly spawned: ${playerId}`);
      return;
    }
    ...
    this.registerTimer(
      setTimeout(() => {
        if (!this.isDestroyed && freezeEffect.isSpawned) {
  ...
  private executeFireball(playerId: string): void {
    ...
    if (!playerEntity.isSpawned || !playerEntity.world) {
      console.error(`🔥 FIREBALL: Player entity not properly spawned: ${playerId}`);
      return;
    }
    ...
    // Play fireball launch sound (using safe audio methods)
    ...
    // Play continuous burning sound that follows the fireball (using safe methods)
    const burnAudio = this.safeCreateAudio("audio/sfx/fire/fire-burning.mp3", {
      loop: true,
      volume: 0.5,
      attachedToEntity: fireball,
      referenceDistance: 12
    });
    this.safePlayAudio(burnAudio, this.world);
    ...
    if (burnAudio) {
      ...
      console.warn(`🔥 FIREBALL: Skipping tracking due to audio creation failure`);
    }
  ...
  private triggerFireballExplosion(explosionPos: { x: number, y: number, z: number }, playerId: string): void {
    ...
    this.registerTimer(
      setTimeout(() => {
        if (!this.isDestroyed && explosionEffect.isSpawned) {
  ...
  private executeSpeedBoost(playerId: string): void {
    ...
    this.addEnhancement(playerId, 'speed', 15000);
  ...
  // Execute enhanced power-up by directly giving it to player (SAFE VERSION)
  private executeEnhancedPowerUp(playerId: string, powerUpName: string): void {
    if (this.isDestroyed) {
      console.warn("🌟 ENHANCED POWER-UP: Manager destroyed, skipping execution");
      return;
    }
    ...
    if (!playerEntity.isSpawned || !playerEntity.world) {
      console.error(`🌟 ENHANCED POWER-UP: Player entity not properly spawned: ${playerId}`);
      return;
    }

    // CRITICAL: Replace dangerous dynamic imports with safe notification system
    console.warn(`⚠️ ENHANCED POWER-UP: Dynamic import system disabled for stability. Power-up ${powerUpName} not available.`);

    // Send user-friendly notification instead of trying to create enhanced abilities
    try {
      if (playerEntity.player.ui && typeof playerEntity.player.ui.sendData === 'function') {
        playerEntity.player.ui.sendData({
          type: "power-up-feedback",
          feedbackType: "error",
          title: "Enhanced Power-Up Unavailable",
          message: `${powerUpName} is temporarily disabled for server stability. Use basic power-ups instead.`,
          duration: 5000
        });
      }
    } catch (uiError) {
      console.error("❌ ENHANCED POWER-UP: Failed to send UI notification:", uiError);
    }

    console.log(`🌟 ENHANCED: ${powerUpName} disabled for stability - player ${playerId} notified`);
  }

  // REMOVED: createEnhancedAbility method - replaced with safer notification system to prevent crashes
  ...
  private executeShuriken(playerId: string): void {
    ...
    const shurikenAudio = new Audio({
      uri: "audio/sfx/player/bow-01.mp3", // This file exists in assets
      loop: false,
      volume: 0.6,
      position: playerEntity.position,
      referenceDistance: 15
  ...
  private findPlayerEntity(playerId: string): SoccerPlayerEntity | null {
    ...
    for (const entity of playerEntities) {
      if (entity instanceof SoccerPlayerEntity) {
        // SAFETY CHECK: Ensure player object exists before accessing properties
        if (!entity.player) {
    ...
    for (const entity of playerEntities) {
      if (entity instanceof SoccerPlayerEntity) {
        if (entity.player) {
          ...
          console.warn(`  - Entity ${entity.id}: No player object attached`);
        }
      }
    }
    return null;
  ...
  private createChargingEffect(player: SoccerPlayerEntity, powerUpType: string): void {
    ...
    let soundUri = chargeSounds[powerUpType] || chargeSounds['default'];

    // Fix missing audio files
    if (soundUri === 'audio/sfx/ui/menu-button-click.mp3') {
      soundUri = 'audio/sfx/ui/button-click.mp3';
    }
